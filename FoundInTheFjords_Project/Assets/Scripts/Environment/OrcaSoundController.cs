...
public class OrcaSoundController : MonoBehaviour
{
    ...
    private IEnumerator OrcasAreCalling()
    {
        ...
        {
            float pause = Random.Range(2.0f, 5.0f); //this sets up a temporary float variable and assigns it a random decimal number between 2 and 5 (you can specify what you want that to be)
            ...
            yield return new WaitForSeconds(OrcaCalls.instance.callDurations[index]); //waits until the selected clip is finished
